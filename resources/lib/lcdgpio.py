...
class LCDGpio(LcdBase):
  ...
  def SendCommand(self, txt, bCheckRet):
    ret = True

    try:
      for i in txt.split('\n'):
        self.lcd.lcd_display_string(txt[i], i+1)
    except:
      # Something bad happened, abort
      log(LOGERROR, "SendCommand: lcd exception - send")
      return False
    ...
    # if ret is False:
    #   log(LOGWARNING, "Reply to '%s' was '%s'" % (txt.decode(self.m_strLCDEncoding), reply.decode(self.m_strLCDEncoding)))
  ...
  def SetBigDigits(self, strTimeString, bForceUpdate):
    ...
    for i in range(int(iStringOffset), int(iStringLength)):
      if self.m_strDigits[iDigitCount] != strTimeString[i] or bForceUpdate:
        ...
        if strTimeString[i] == ":":
          self.m_bstrSetLineCmds += "%i %i 10\n" % (iDigitCount, iOffset)
        elif strTimeString[i].isdigit():
          self.m_bstrSetLineCmds += "%i %i %s\n" % (iDigitCount, iOffset, strTimeString[i])
        else:
          self.m_bstrSetLineCmds += "%i 0 0\n" % (iDigitCount)
  ...
  def SetPlayingStateIcon(self):
    ...
    self.m_bstrIconName = "STOP"

    if bForwarding:
      self.m_bstrIconName = "FF"
    elif bRewinding:
      self.m_bstrIconName = "FR"
    elif bPaused:
      self.m_bstrIconName = "PAUSE"
    elif bPlaying:
      self.m_bstrIconName = "PLAY"
  ...
  def SetLine(self, mode, iLine, strLine, dictDescriptor, bForce):
    ...
    if len(strLineLong) > iMaxLineLen: # if the string doesn't fit the display...
      if iScrollSpeed != 0:            # add separator when scrolling enabled
        if bstrScrollMode == "m":     # and scrollmode is marquee
    ...
    if dictDescriptor['type'] == LCD_LINETYPE.LCD_LINETYPE_ICONTEXT:
      if self.m_bstrLineIcon[iLine] != self.m_bstrIconName or bExtraForce:
        ...
        self.m_bstrSetLineCmds += "%i 1 %i %s\n" % (ln, ln, self.m_bstrIconName)
